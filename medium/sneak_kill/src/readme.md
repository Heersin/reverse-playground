# sneak_kill
- flag：flag{xxaaaaxxxdddddddwwdddxtxxk} 或者 flag{xxaaaaxxxdddddddwwdddxtxxb}
- 预估难度：低
...
## 解题思路
逆向后编写一个具有同样行为的脚本，运行后能显示当前迷宫图和下一秒的迷宫图，最后通过手动游玩的方式走迷宫。
脚本如下：
```python
MAP_WIDTH = 42
MAP_HEIGHT = 10
...
## 项目结构
```shell
├── debug_sneak.h debug用的函数
├── guard_view.c 地图数据
├── main.c 主程序
├── map_data
│   ├── gen_map_cdata.py 生成地图数据
│   ├── guard1.txt
│   ├── guard2.txt
│   └── map_show.py 辅助走迷宫
├── readme.md
